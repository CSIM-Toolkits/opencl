...
int DoIt( int argc, char * argv[], T )
{
    ...
    */
            ...
            buildData.kernelName = std::string("MeanFilter.cl");
            buildData.kernelPath = std::string("/home/alexandrefreitas/Projetos/Slicer/MeanFilterOpencl/MeanFilter/");

            char buildOption[128];
            sprintf(buildOption, "-D width=%d -D height=%d -D depth=%d -D radius=%d",width,height,deep,meanRadius);
                    ...
                    for(int gix=0; gix<width; gix++){
                        ...
                        for(int j=-step;j<step+1;j++){
                            for(int i=-step;i<step+1;i++){
                                ...
                                neighborhood[count]=index + (width*j) + i;
                                count++;
                            }
                        }
                        ...
                        for(int i=0;i<radius*radius;i++){
                            if(!isnan(neighborhood[i])) sum=sum+pixelData[neighborhood[i]];

                        }


                        pixelDataOut[index] = (float)(sum/num);
...
/*
        textFile textfile;
        textfile.fileName = std::string("ModifiedImage.txt");
        textfile.filePath = std::string("/home/alexandrefreitas/Projetos/Slicer/MeanFilterOpencl/MeanFilter/");

        writeImageTextFile(textfile,pixelDataOut,width,height,1);

        textfile.fileName = std::string("OriginalImage.txt");
        writeImageTextFile(textfile,pixelData,width,height,1);
*/
