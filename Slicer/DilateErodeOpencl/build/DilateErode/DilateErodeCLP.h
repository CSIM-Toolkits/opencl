...
//  /home/alexandrefreitas/MestradoBibliotecas/Slicer/Slicer/Slicer-SuperBuild-Debug/SlicerExecutionModel-build/GenerateCLP/bin/GenerateCLP --InputXML /home/alexandrefreitas/Projetos/Slicer/DilateErodeOpencl/DilateErode/DilateErode.xml --OutputCxx /home/alexandrefreitas/Projetos/Slicer/DilateErodeOpencl/build/DilateErode/DilateErodeCLP.h
//
#include <cstdio>
#include <cstdlib>
#include <iostream>
...
#define GENERATE_TCLAP_PARSE \
    ...
    std::string filterType = "Dilate"; \
    std::vector<std::string> filterTypeAllowed; \
    filterTypeAllowed.push_back("Dilate");  \
    filterTypeAllowed.push_back("Erode");  \
    TCLAP::ValuesConstraint<std::string> filterTypeAllowedVals (filterTypeAllowed);  \
 ...
 \
    msg.str("");msg << "Select a pixel value foreground (default: " << foreGround << ")"; \
    TCLAP::ValueArg<int > foreGroundArg("", "foreGround", msg.str(), 0, foreGround, "int", commandLine); \
 \
    msg.str("");msg << "Select a pixel value for background (only erode filter) (default: " << backGround << ")"; \
    TCLAP::ValueArg<int > backGroundArg("", "backGround", msg.str(), 0, backGround, "int", commandLine); \
 \
    msg.str("");msg << "Select a radius of kernel (default: " << radiusIn << ")"; \
    TCLAP::ValueArg<int > radiusInArg("", "radiusIn", msg.str(), 0, radiusIn, "int", commandLine); \
 \
    msg.str("");msg << "Boundary condition (default: " << boundary << ")"; \
    TCLAP::SwitchArg boundaryArg("", "boundary", msg.str(), commandLine, boundary); \
 \
 ...
 \
    msg.str("");msg << "Select the type (dilate or erode) (default: " << filterType << ")"; \
    TCLAP::ValueArg<std::string> filterTypeArg("", "filtertype", msg.str(), 0, filterType, &filterTypeAllowedVals, commandLine); \
 \
...
#define GENERATE_TCLAP_ASSIGNMENT \
      ...
      foreGround = foreGroundArg.getValue(); \
      backGround = backGroundArg.getValue(); \
      radiusIn = radiusInArg.getValue(); \
      boundary = boundaryArg.getValue(); \
      deviceType = deviceTypeArg.getValue(); \
      filterType = filterTypeArg.getValue(); \
...
#define GENERATE_TCLAP_ASSIGNMENT_IFSET \
    ...
    if( radiusInArg.isSet() ) \
      { \
      radiusIn = radiusInArg.getValue(); \
      } \
    if( boundaryArg.isSet() ) \
      { \
      boundary = boundaryArg.getValue(); \
      } \
