...
/*
 ...
 for(int gix=0; gix<width; gix++){
	...
	if( isValid )
	{
	    ...
	    {
	      unsigned int zid = (unsigned int)(min(max(0, z),(int)(deep-1)));
	      for(int y = giy-radiusy; y <= giy+radiusy; y++)
	      {
		unsigned int yid = (unsigned int)(min(max(0, y),(int)(height-1)));
		for(int x = gix-radiusx; x <= gix+radiusx; x++)
		{
		  unsigned int cidx = width*(zid*height + yid) + (unsigned int)(min(max(0, x),(int)(width-1)));

		  sum += (float)pixelData[cidx];
		}
	      }
	    }

		pixelDataOut[gidx] = (float)(sum/(float)num);
