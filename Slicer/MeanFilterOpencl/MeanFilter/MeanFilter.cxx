...
size_t blockSizeZ;                             /**< Work-group size in y-direction */
...
int Allocate(std::string inputVolume){
    ...
    BuildOption radiusOption;
    radiusOption.name = "radius";
    radiusOption.value = &radius;
    radiusOption.type = TYPE_INT;
    vectOption.push_back(radiusOption);
...
int DoIt( int argc, char * argv[], T )
{
    ...
    gputimer.Stop();
    std::cout << "ALLOCATE took " << gputimer.GetTotal() << " seconds with\n" << std::endl;
    ...
    if(deviceType.compare("GPU")==0){
        ...
        if(LoadKernelAndCompile("/home/alexandrefreitas/Projetos/Slicer/MeanFilterOpencl/MeanFilter/MeanFilter.cl","MeanFilter")!= SDK_SUCCESS){
            return SDK_FAILURE;
        }
            ...
            for(int giy=0; giy<height; giy++){
                for(int gix=0; gix<width; gix++){
                    ...
                    for(int z=0;z<radius*radius;z++){
                        sum=sum+pixelData[neighborhood[z]];
    ...
    gputimer.Stop();
    std::cout << "WRITER UPDATE took " << gputimer.GetTotal() << " seconds with\n" << std::endl;
