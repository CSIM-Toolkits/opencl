...
#define GENERATE_TCLAP_PARSE \
    ...
    deviceTypeAllowed.push_back("CPU ITK");  \
    deviceTypeAllowed.push_back("CPU PROCEDURAL");  \
    TCLAP::ValuesConstraint<std::string> deviceTypeAllowedVals (deviceTypeAllowed);  \
    int numInterationInput = 1; \
    float kappaInput = 15.0; \
    float deltaInput = 0.125; \
 ...
 \
    msg.str("");msg << "Select the number of interations (default: " << numInterationInput << ")"; \
    TCLAP::ValueArg<int > numInterationInputArg("", "numInteration", msg.str(), 0, numInterationInput, "int", commandLine); \
 \
    msg.str("");msg << "Select the value of coef. Kappa (default: " << kappaInput << ")"; \
    TCLAP::ValueArg<float > kappaInputArg("", "kappa", msg.str(), 0, kappaInput, "float", commandLine); \
 \
    msg.str("");msg << "Select the value of Delta (default: " << deltaInput << ")"; \
    TCLAP::ValueArg<float > deltaInputArg("", "delta", msg.str(), 0, deltaInput, "float", commandLine); \
 \
...
#define GENERATE_TCLAP_ASSIGNMENT \
      ...
      numInterationInput = numInterationInputArg.getValue(); \
      kappaInput = kappaInputArg.getValue(); \
      deltaInput = deltaInputArg.getValue(); \
...
#define GENERATE_TCLAP_ASSIGNMENT_IFSET \
    ...
    if( numInterationInputArg.isSet() ) \
      { \
      numInterationInput = numInterationInputArg.getValue(); \
      } \
    if( kappaInputArg.isSet() ) \
      { \
      kappaInput = kappaInputArg.getValue(); \
      } \
    if( deltaInputArg.isSet() ) \
      { \
      deltaInput = deltaInputArg.getValue(); \
      } \
...
std::cout << "    numInterationInput: " << numInterationInput << std::endl; \
std::cout << "    kappaInput: " << kappaInput << std::endl; \
std::cout << "    deltaInput: " << deltaInput << std::endl; \
