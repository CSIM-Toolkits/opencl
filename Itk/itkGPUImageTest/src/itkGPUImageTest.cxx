...
int main(int argc, char *argv[])
{
  ...
  typedef itk::ImageLinearIteratorWithIndex< ItkImage1f >      IteratorType;
  typedef itk::ImageFileReader< ItkImage1f  >  ReaderType;
  ...
  ItkImage1f::Pointer image = reader->GetOutput();

std::cout<<"Teste"<<image<<std::endl;
  ...
  for ( inputIt.GoToBegin();  ! inputIt.IsAtEnd();  inputIt.NextLine())
    ...
    while ( ! inputIt.IsAtEndOfLine() )
      ...
      //std::cout<<"Pixel Value"<< inputIt.Get() << std::endl;
      ++inputIt;
	if(max < inputIt.Get() ){
		max = inputIt.Get();
	}
...
std::cout<<"Máximo: "<< max << std::endl;
  ...
  //std::cout << "Código Kernel " << GPUSource << std::endl;

  kernelManager->LoadProgramFromFile("/home/alexandrefreitas/Projetos/ITKProjects/itkGPUImageTest/src/GPUImageOps.cl", "#define PIXELTYPE float\n");
  ...
  std::cout << "SrcA : " << image->GetPixel( idx ) << std::endl;
  std::cout << "SrcB : " << srcB->GetPixel( idx ) << std::endl;
  std::cout << "Dest : " << dest->GetPixel( idx ) << std::endl;

  kernelManager->SetKernelArgWithImage(kernel_add, 0, image->GetGPUDataManager());
