...
int runGPUMeanImageFilterTest(const std::string& inFile, const std::string& outFile)
{
  ...
  //Apenas declara a classe
  typedef itk::GPUMyMeanImageFilter< InputImageType, OutputImageType > GPUMyMeanFilterType;

  //Instanciação da classe, para ser possivel usar seus metodos
  //typename GPUMyMeanFilterType::Pointer GPUFilter2 = GPUMyMeanFilterType::New();
  ...
  for(int nThreads = 1; nThreads <= 8; nThreads++)
  {
    ...
    if( nThreads == 8 )
    {
      ...
      GPUFilter2->SetInput( reader->GetOutput() );
      GPUFilter2->SetRadius( indexRadius );
      //GPUFilter2->Update();
      //GPUFilter2->GetOutput()->UpdateBuffers(); // synchronization point (GPU->CPU memcpy)

      gputimer.Stop();
      std::cout << "GPU mean filter took " << gputimer.GetMean() << " seconds.\n" << std::endl;

      writer->SetInput( GPUFilter2->GetOutput() );
      writer->Update();
    }
  }
