...
int main() {
   ...
   /* Extension data */
   char name_data[48], ext_data[4096],vendor_data[100],ver_data[50];
   ...
   clGetDeviceInfo(dev, CL_DEVICE_VENDOR, 		
      100 * sizeof(char), vendor_data, NULL);

   /* Access device extensions */
   clGetDeviceInfo(dev, CL_DRIVER_VERSION, 		
      50 * sizeof(char), ver_data, NULL);
			
   printf("GPU\n");
//   printf("NAME: %s\nADDRESS_WIDTH: %u\nEXTENSIONS: %s\nVENDOR: %s\n", 
//    name_data, addr_data, ext_data,vendor_data);

   printf("NAME: %s\nADDRESS_WIDTH: %u\nVERSION: %s\nVENDOR: %s\n", 
     name_data, addr_data, ver_data,vendor_data);
